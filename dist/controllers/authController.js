                ...
                _connection2.default.query('SELECT * FROM users WHERE email=$1', [email]).then(function (result) {
                    if (result.rowCount < 1) {
                        ...
                        if (!comparePassword) {
                            ...
                            _jsonwebtoken2.default.sign({ user: user }, process.env.JWT_SECRET_TOKEN, { expiresIn: '20h' }, function (error, token) {
                                if (error) {
                                    res.status(522).json({
                                        status: 'error',
                                        message: 'Auth Failed!',
        ...
        value: function register(req, res) {
            ...
            _connection2.default.query('SELECT * FROM users WHERE email=$1', [email]).then(function (result) {
                if (result.rowCount >= 1) {
                    ...
                    var userData = [uid, name, email, ecryptedPassword, 'avatar.png', new Date().toISOString()];
                    var query = 'INSERT INTO users(id, name, email, password, photo, created_at) VALUES ($1, $2, $3, $4, $5, $6) RETURNING *';
