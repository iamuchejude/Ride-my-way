  ...
  _createClass(Rides, null, [{
    ...
    value: function createRideOffer(req, res) {
      var _req$body2 = req.body,
          ...
          startFrom = _req$body2.startFrom,
          destination = _req$body2.destination,
          price = _req$body2.price,
          seat = _req$body2.seat,
          departureDate = _req$body2.departureDate,
          departureTime = _req$body2.departureTime;


      if (userId === undefined || userId.trim().length < 1 || startFrom === undefined || startFrom.trim().length < 1 || destination === undefined || destination.trim().length < 1 || price === undefined || price.trim().length < 1 || seat === undefined || seat.trim().length < 1 || departureDate === undefined || departureDate.trim().length < 1 || departureTime === undefined || departureTime.trim().length < 1) {
        res.status(409).json({
          status: 'error',
        ...
        _connection2.default.query("SELECT * FROM ride_offers WHERE user_id=$1, departure_date=$2 AND departure_time=$3", checkData).then(function (res) {
          if (res.rowCount > 0) {}
        }).catch(function (err) {
          res.status(500).json({
            status: 'error',
