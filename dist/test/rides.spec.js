...
describe('Test for ride endpoints for Ride-my-way api', function () {
  ...
  describe('GET one ride offer', function () {
    it('should return an object of a ride offer', function (done) {
      _chai2.default.request(_app2.default).get('/api/v1/rides/b4a73be0-805e-11e8-8b74-375385422771').set('Accept', 'application/json').set('Content-Type', 'application/x-www-form-urlencoded').set('Authorization', 'Bearer ' + token).end(function (err, res) {
        (0, _chai.expect)(err).to.equal(null);
        (0, _chai.expect)(res.status).to.equal(200);
        (0, _chai.expect)(res.body.status).to.equal('success');
        (0, _chai.expect)(res.body.ride).to.be.an('object');
        done();
      });
    });
  });
  ...
  describe('POST ride offer request', function () {
    it('should respond with success in an object also containing created resource', function (done) {
      _chai2.default.request(_app2.default).post('/api/v1/rides/b4a73be0-805e-11e8-8b74-375385422771/requests').set('Accept', 'application/json').set('Content-Type', 'application/x-www-form-urlencoded').set('Authorization', 'Bearer ' + token).send({
        userId: '5f1314c0-805d-11e8-8b74-375385422771'
      }).end(function (err, res) {
        (0, _chai.expect)(err).to.equal(null);
        (0, _chai.expect)(res.status).to.equal(201);
        (0, _chai.expect)(res.body.status).to.equal('success');
        (0, _chai.expect)(res.body.request).to.be.an('object');
  ...
  describe('GET all requests for one ride offer', function () {
    it('should respond with success in an object also containing an array of all requests for ride offer', function (done) {
      _chai2.default.request(_app2.default).get('/api/v1/users/rides/b4a73be0-805e-11e8-8b74-375385422771/requests').set('Accept', 'application/json').set('Content-Type', 'application/x-www-form-urlencoded').set('Authorization', 'Bearer ' + token).end(function (err, res) {
        (0, _chai.expect)(err).to.equal(null);
        (0, _chai.expect)(res.status).to.equal(200);
        (0, _chai.expect)(res.body.status).to.equal('success');
        (0, _chai.expect)(res.body.requests).to.be.an('array');
